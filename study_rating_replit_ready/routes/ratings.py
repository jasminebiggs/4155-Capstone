...
async def rate_partner(data: RatingInput):
    ...
    existing = await database.fetch_one(
        "SELECT id FROM ratings WHERE session_id = :session_id AND reviewer_id = :reviewer_id",
    ...
    await database.execute(
        "INSERT INTO ratings (session_id, reviewer_id, partner_id, rating, feedback) VALUES (:session_id, :reviewer_id, :partner_id, :rating, :feedback)",
        data.dict()
    )
